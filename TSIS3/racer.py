...
def run_game(screen, settings, username):
    ...
    while running:
        ...
        text_dist = font.render(f"Dist: {int(distance)}m", True, BLACK)
        ...
        if hit_obs or hit_enemy:
            ...
            if active_powerup == "Shield":
                ...
                if hit_obs and hit_obs.type == 0:
                    base_speed = max(2, base_speed - 2) # Oil spill slows you down instead of killing if you survive? 
                    # Wait, requirement says "collision ends the run". Oil spill is obstacle. Let's make it end run unless it's just slow-down.
                    # "obstacles, oil spills, or slow-down zones in some lanes so the player must choose the safest path."
                    # Let's say Oil Spill just slows down and deducts 0 lives? No, it says "restores one crash or clears one obstacle: instant". So obstacles are crashes.
                    pass
                hit_sprite.kill()
                if lives <= 0:
