    ...
    def update(self):
        ...
        if self.poison_food and self.snake_pos == self.poison_food.pos:
            if len(self.snake_body) > 4:
                # Отрезаем 2 сегмента. Переменную ate_food не трогаем (остается False), 
                # поэтому ниже код сделает еще один pop() для обычного шага. Итог: -2 сегмента.
        ...
        if collision:
            if self.shield_active:
                self.shield_active = False
                # Отменяем шаг (убираем только что вставленную голову)
